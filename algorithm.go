...
// AlgorithmRun 执行算法
func (c *Client) AlgorithmRun(ctx context.Context, projectId, id string, data interface{}) ([]byte, error) {
	if projectId == "" {
	...
	if data == nil {
		return nil, errors.NewMsg("更新数据为空")
	}
	bts, err := json.Marshal(data)
	if err != nil {
		return nil, errors.NewMsg("marshal 更新数据为空")
	}
	res, err := cli.Run(
		apicontext.GetGrpcContext(ctx, map[string]string{config.XRequestProject: projectId}),
		&algorithm.ClientRunRequest{Id: id, Data: bts})
	if err != nil {
