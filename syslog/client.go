...
type Client struct {
	...
	syslogClient SyslogServiceClient
}
...
func NewLocalClient(cfg config.Config, cc ggrpc.ClientConnInterface) (*Client, func(), error) {
	...
	c.syslogClient = NewSyslogServiceClient(cc)
	cleanFunc := func() {}
	return c, cleanFunc, nil
}
...
func (c *Client) GetSyslogServiceClient() (SyslogServiceClient, error) {
	if c.syslogClient != nil {
		return c.syslogClient, nil
	}
	if c.conn == nil {
		if err := c.createConn(); err != nil {
			return nil, err
